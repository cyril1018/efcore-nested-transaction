public class NestedTransactionManager: IDbContextTransactionManager
{
    ...
    {
        if (Layer++ == 0) _sqlServerConnection.BeginTransaction();
    ...
    public async Task < IDbContextTransaction > BeginTransactionAsync(CancellationToken cancellationToken =
        default)
    {
        if (Layer++ == 0) await _sqlServerConnection.BeginTransactionAsync(cancellationToken);
        return new NestedTransation(this, Layer);
    }
	
    public void CommitTransaction()
    {
        if (Layer-- <= 1) _sqlServerConnection.CommitTransaction();
    }
